...
TOTAL_DRAWS=$(tail -n +2 "$HISTORY_FILE" | awk -F',' \
    '$1=="draw" && $2=="draw" {count++} END {print count+0}')
...
# Print per-player metric header
printf "  %-14s" "Player"
while IFS= read -r game; do
...
echo ""
echo "╠══════════════════════════════════════════════════╣"
echo "  Sorted by: $SORT_METRIC | Total games played: $TOTAL_GAMES [Total Draws = $TOTAL_DRAWS]"
echo "╚══════════════════════════════════════════════════╝"
echo ""
